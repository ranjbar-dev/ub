...
class RsaKeyHelper {
  ...
  String sign(String plainText, RSAPrivateKey privateKey) {
    ...
    // var signer = RSASigner(SHA256Digest(), "6592058374869485869036");
...
class EncryptModel {
  ...
  EncryptModel(this.plaintext, this.publicKey);
}

/// Encrypting String
String encrypt({@required EncryptModel encryptModel}) {
  var cipher = new RSAEngine()
    ..init(true, new PublicKeyParameter<RSAPublicKey>(encryptModel.publicKey));
  var cipherText =
      cipher.process(new Uint8List.fromList(encryptModel.plaintext.codeUnits));
...
class DecryptModel {
  ...
  DecryptModel(this.ciphertext, this.privateKey);
}

/// Decrypting String
String decrypt({@required DecryptModel decryptModel}) {
  var cipher = new RSAEngine()
    ..init(
        false, new PrivateKeyParameter<RSAPrivateKey>(decryptModel.privateKey));
  var decrypted =
      cipher.process(new Uint8List.fromList(decryptModel.ciphertext.codeUnits));
