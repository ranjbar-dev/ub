...
class UniversalMqttClient {
  ...
  Future<void> connect() async {
    try {
      ...
      await _mqtt.connect(_username, _password).catchError((e) {
        // debugPrint(e.toString());
        debugPrint('error connecting to mqtt');
      }).timeout(
        timeout,
        onTimeout: () async {
          _mqtt.disconnect();
  ...
  MqttSubscriptionStatus getSubscriptionsStatus({@required String topic}) {
    return _mqtt.getSubscriptionsStatus(topic);
  }
