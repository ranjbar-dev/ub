import React from 'react';
export default function CloseIcon() {
  return (
    <svg id="Group_7520" width="24" height="24" viewBox="0 0 24 24">
      <defs>
        <clipPath id="clip-pathclose">
          <rect id="Rectangle_4028" width="24" height="24" fill="#818181" />
        </clipPath>
      </defs>
      <g id="Group_7519" clipPath="url(#clip-pathclose)">
        <path
          id="Path_7127"
          d="M1523.327,550.886l5.3-5.3a.5.5,0,0,0-.708-.707l-5.3,5.3-5.3-5.3a.5.5,0,0,0-.707.707l5.3,5.3-5.3,5.3a.5.5,0,1,0,.707.707l5.3-5.3,5.3,5.3a.5.5,0,1,0,.708-.707Z"
          transform="translate(-1510.619 -538.886)"
          fill="#818181"
        />
      </g>
    </svg>
  );
}
