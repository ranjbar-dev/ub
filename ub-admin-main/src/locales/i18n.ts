...
export const translations: ConvertedToFunctionsType<TranslationResource> = {} as ConvertedToFunctionsType<TranslationResource>;
 ...
 */
const convertToFunctions = (obj: Record<string, unknown>, dict: Record<string, unknown>, current?: string) => {
  Object.keys(obj).forEach(key => {
    const currentLookupKey = current ? `${current}.${key}` : key;
    if (typeof obj[key] === 'object') {
      dict[key] = {};
      convertToFunctions(obj[key] as Record<string, unknown>, dict[key] as Record<string, unknown>, currentLookupKey);
    } else {
