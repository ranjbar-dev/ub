...
const DepositePage = props => {
  ...
  useEffect(() => {
    ...
    dispatch(
      getDepositAndWithDrawDataAction({ code: 'BTC', type: 'withdraw' }),
    );
  ...
  return (
    <Wrapper>
      <AnimatedChildren isLoading={isLoadingDepositeAndWithdraw}>
        <HalfWrapper1>
          <TitledComponent
            title={<FormattedMessage {...translate.withdrawalAddress} />}
          >
            <WithdrawalAddress />
          </TitledComponent>
        </HalfWrapper1>
        <HalfWrapper2>
          <TitledComponent
            ...
            title={<FormattedMessage {...translate.withdrawalHistory} />}
          >
            <DataGrid
              coinCode={localStorage[LocalStorageKeys.SELECTED_COIN] ?? 'BTC'}
              data={
                depositAndWithDrawData.withdrawTransactions
                  ? depositAndWithDrawData.withdrawTransactions
                  : []
              }
              sectionName='withdraw'
            />
          </TitledComponent>
        </HalfWrapper2>
      </AnimatedChildren>
